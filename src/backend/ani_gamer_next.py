...
def build_anime(sn):
    ...
    try:
        if settings.use_gost:  # use_gost 是運行時計算的
            # 如果使用 gost, 则随机一个 gost 监听端口
            # 使用 globals() 避免與函數名衝突
            _gost_port_fn = globals().get("gost_port")
            if callable(_gost_port_fn):
                port: int = _gost_port_fn()
            else:
                port = 34173  # 默認端口
            anime["anime"] = Anime(sn, gost_port=port)
        else:
    ...
    # sn 解析冷却
    if cfg.parse_sn_cd > 0:
        err_print(
            "更新資訊",
            f"SN 解析冷卻 {cfg.parse_sn_cd} 秒",
            no_sn=True,
        )
        time.sleep(cfg.parse_sn_cd)
...
def insert_db(anime):
    ...
    try:
        ...
        err_print(
            anime_dict["sn"],
            "ＤＢ错误",
            "title=" + anime_dict["title"] + " 数据已存在！" + str(e),
            status=1,
        )
...
def worker(sn, sn_info, realtime_show_file_size=False):
    ...
    if (
        cfg.upload_to_server
        and anime_in_db["status"] == 1
        and anime_in_db["remote_status"] == 0
    ):
    ...
    if anime.video_size < 5:
        ...
        thread_limiter.release()
        err_msg_detail = (
            'title="' + anime.get_title() + '" 從任務列隊中移除, 等待下次更新重試.'
        )
        err_print(sn, "任务失敗", err_msg_detail, status=1)
        if int(sn) in config.tasks_progress_rate.keys():
            # 記錄失敗的任務
            filename = config.tasks_progress_rate[int(sn)].get("filename", f"SN: {sn}")
            config.record_completed_task(int(sn), filename, "failed")
...
def download_cd_counter():
    cfg = config.get_config()
    seconds = cfg.download_cd
    while seconds > 0:
        err_print(
            "",
            "下載冷卻:",
            "下載冷卻時間剩餘 " + str(seconds) + " 秒",
            status=0,
            no_sn=True,
        )
...
def check_tasks():
    ...
    for sn in sn_dict.keys():
        ...
        if anime["failed"]:
            ...
            # # sn 解析冷却
            # if cfg.parse_sn_cd > 0:
            #     err_print("更新資訊", "SN 解析冷卻 " + str(cfg.parse_sn_cd) + " 秒", no_sn=True)
            #     time.sleep(cfg.parse_sn_cd)
            continue
        anime = anime["anime"]
        ...
        if sn_dict[sn]["mode"] == "all":
            ...
            for ep in episode_list:  # 遍历剧集列表
                try:
                    ...
                    if (
                        db["status"] == 0
                        or (db["remote_status"] == 0 and cfg.upload_to_server)
...
def __download_only(
    sn, dl_resolution="", dl_save_dir="", realtime_show_file_size=False, classify=True
):
    # 仅下载,不操作数据库
    cfg = config.get_config()
    
    # 等待獲取線程資源（這期間任務顯示在"等待中"）
    thread_limiter.acquire()
    
    # 標記手動任務已開始執行
    config.start_manual_task(int(sn))
    ...
    if anime["failed"]:
        # 任務失敗，從手動任務隊列中移除
        config.remove_manual_task(int(sn))
        thread_limiter.release()
        sys.exit(1)
    anime = anime["anime"]
    
    # 獲取到線程資源後，創建進度條目（移到"執行中"）
    config.tasks_progress_rate[int(sn)] = {
        "rate": 0,
        "filename": f"《{anime.get_title()}》",
        "status": "正在解析"
    }
...
def __get_info_only(sn):
    ...
    anime.get_info()
    download_dir = cfg.bangumi_dir
    if classify:  # 控制是否建立番剧文件夹
        download_dir = os.path.join(
            download_dir, config.legalize_filename(anime.get_bangumi_name())
        )

    if danmu:
        if os.path.exists(download_dir):
            full_filename = os.path.join(download_dir, anime.get_filename()).replace(
                "." + cfg.video_filename_extension, ".ass"
            )
            d = Danmu(sn, full_filename, config.read_cookie())
            d.download(cfg.danmu_ban_words)
        else:
...
def __get_danmu_only(sn, bangumi_name, video_path):
    ...
    if classify:  # 控制是否建立番剧文件夹
        download_dir = os.path.join(
            download_dir, config.legalize_filename(bangumi_name)
        )

    if os.path.exists(download_dir):
        d = Danmu(
            sn,
            video_path.replace("." + cfg.video_filename_extension, ".ass"),
            config.read_cookie(),
        )
        d.download(cfg.danmu_ban_words)
    else:
...
def __cui(
    ...
    # 使用全局的 thread_limiter，確保所有任務（手動+自動）共享同一個並發控制
    # 這樣不會出現新任務插隊或舊任務永遠等待的問題
    global danmu
    danmu = cui_danmu

    if realtime_show:
        if cui_thread_limit == 1 or cui_download_mode in (
            "single",
            "latest",
            "largest-sn",
        ):
    ...
    if cui_download_mode == "single":
        ...
        # 先將所有任務添加到手動任務隊列（顯示在"等待中"）
        if not get_info:
            for anime_sn in bangumi_list:
                config.add_manual_task(
                    int(anime_sn),
                    f"SN: {anime_sn}",  # 先用簡單文件名，後面會更新
                    "all"
                )
        ...
        for ep in ep_range:
            if ep in bangumi_ep_list:
                if get_info:
                    a = threading.Thread(
                        target=__get_info_only, args=(episode_dict[ep],)
                    )
                else:
                    a = threading.Thread(
                        target=__download_only,
                ...
                err_print(
                    0,
                    "《" + anime.get_bangumi_name() + "》 第 " + ep + " 集不存在!",
                    status=1,
                    no_sn=True,
                )
        print(
        ...
        for sn in ep_range:
            if get_info:
                ...
                a = threading.Thread(
                    target=__download_only,
                    args=(sn, cui_resolution, cui_save_dir, realtime_show_file_size),
                )
            a.daemon = True
            thread_tasks.append(a)
            a.start()
        ...
        if get_info:
            ...
            for sn in queue.keys():  # 遍历任务列队
                ...
                task = threading.Thread(
                    target=worker, args=(sn, queue[sn], realtime_show_file_size)
                )
                task.daemon = True
                thread_tasks.append(task)
                task.start()
        ...
        for anime_db in ep_range:
            if anime_db["status"] == 1:
                if (
                    anime_db["anime_name"] is not None
                    and anime_db["local_file_path"] is not None
                ):
                    a = threading.Thread(
                        target=__get_danmu_only,
                        args=(
                            anime_db["sn"],
                            anime_db["anime_name"],
                            anime_db["local_file_path"],
                        ),
                    )
                    a.daemon = True
                    thread_tasks.append(a)
...
def __init_proxy():
    if settings.use_gost:
        ...
        # 需要使用 gost 的情况
        # 寻找 gost
        check_gost = subprocess.Popen(
            "gost -h", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        if (
            check_gost.stderr and check_gost.stderr.readlines()
        ):  # 查找 gost 是否已放入系统 path
            gost_path = "gost"
        else:
        ...
        def run_gost():
            ...
            gost_subprocess = subprocess.Popen(
                gost_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
...
def parse_anime(soup, animes, headers, cookies):
    ...
    for animeInfo in soup.select_one(".theme-list-block").select("a"):
        response = do_request(
            f"https://ani.gamer.com.tw/{animeInfo['href']}", headers, cookies
        )
...
def export_my_anime():
    ...
    while True:
        ...
        bahamygatherPage = do_request(
            url, headers=header, cookies=cookies, params=params
        )
        if bahamygatherPage.status_code == 200:
            ...
            err_print(
                0,
                f"匯入我的動畫失敗，狀態碼{bahamygatherPage.status_code}",
                status=1,
                no_sn=True,
            )
        page += 1
...
def run_dashboard():
    cfg = config.get_config()
    # 检测端口是否占用
    if not port_is_available(cfg.dashboard.port):
        err_print(
            0,
            "Web控制面板啟動失敗",
            "Port已被占用! 請到配置文件更換",
    ...
    from dashboard_server import run as dashboard

    server = threading.Thread(target=dashboard)
    server.daemon = True
    server.start()
    if cfg.dashboard.SSL:
    ...
    dashboard_address = dashboard_address + host + ":" + str(cfg.dashboard.port)
...
processing_queue = []
thread_limiter = threading.Semaphore(cfg.multi_thread)  # 下载并发限制器
upload_limiter = threading.Semaphore(cfg.multi_upload)  # 并发上传限制器
...
# 設置任務佇列引用，供 Dashboard 使用
config.set_task_queue_refs(queue, processing_queue)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:  # 支持命令行使用
        ...
        parser.add_argument(
            "--thread_limit", "-t", type=int, help="最高併發下載數(數字)"
        )
        parser.add_argument(
            "--current_path", "-c", action="store_true", help="下載到當前工作目錄"
        )
        parser.add_argument("--episodes", "-e", type=str, help="僅下載指定劇集")
        parser.add_argument(
            "--no_classify", "-n", action="store_true", help="不建立番劇資料夾"
        )
        parser.add_argument(
            "--user_command",
            "-u",
        ...
        parser.add_argument(
            "--danmu", "-d", action="store_true", help="以 .ass 下載彈幕"
        )
        parser.add_argument(
            "--my_anime", action="store_true", help="匯出「我的動畫」至my_anime.txt"
        )
        ...
        if (
            arg.download_mode not in ("list", "multi", "sn-list", "db")
        ) and arg.sn is None:
            err_print(
                0, "參數錯誤", "非 list/multi 模式需要提供 sn ", no_sn=True, status=1
            )
            sys.exit(1)
        ...
        if arg.current_path:
            ...
            print(info + "\n    " + cfg.bangumi_dir)
            err_print(0, info + cfg.bangumi_dir, no_sn=True, display=False)
        ...
        if not arg.episodes and arg.download_mode == "range":
            err_print(
                0, "ERROR: 當前為指定範圍模式, 但範圍未指定!", status=1, no_sn=True
            )
            sys.exit(1)
            ...
            if arg.download_mode == "multi":
                ...
                for i in arg.episodes.split(","):
                    if re.match(r"^\d+-\d+$", i):
                        ...
                        if (
                            episodes_range_start > episodes_range_end
                        ):  # 如果有zz从大到小写
                            episodes_range_start, episodes_range_end = (
                                episodes_range_end,
                                episodes_range_start,
                            )
                        download_episodes.extend(
                            list(range(episodes_range_start, episodes_range_end + 1))
                        )
                    if re.match(r"^\d+$", i):
                        download_episodes.append(int(i))
        ...
        if not arg.resolution:
            resolution = cfg.download_resolution
    ...
    while True:
        ...
        config.test_cookie()  # 测试cookie
        if cfg.read_sn_list_when_checking_update:
            sn_dict = config.read_sn_list()
        if cfg.read_config_when_checking_update:
            settings = config.get_settings()
        ...
        if queue:
            for task_sn in queue.keys():
                if task_sn not in processing_queue:  # 如果该任务没有在进行中，则启动
                    # 立即在 tasks_progress_rate 中創建條目，避免前端顯示跳躍
                    try:
                        db = read_db(task_sn)
                        filename = f"《{db['anime_name']}》- {db['episode']}"
                    except (IndexError, KeyError, Exception):
                        filename = f"SN: {task_sn}"
                    
                    config.tasks_progress_rate[int(task_sn)] = {
                        "rate": 0,
                        "filename": filename,
                        "status": "等待下載"
                    }
                    ...
                    task = threading.Thread(
                        target=worker, args=(task_sn, queue[task_sn])
                    )
                    task.daemon = True
                    task.start()
        ...
        info = (
            "本次更新添加了 "
            + str(new_tasks_counter)
            + " 個新任務, 目前列隊中共有 "
            + str(len(processing_queue))
            + " 個任務"
        )
        err_print(0, "更新資訊", info, no_sn=True)
        err_print(0, "更新终了", no_sn=True)
        print()
        for i in range(cfg.check_frequency * 60):
