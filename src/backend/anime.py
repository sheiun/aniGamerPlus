...
此模組提供動畫瘋影片的下載、上傳等核心功能。

Classes:
    TryTooManyTimeError: 重試次數過多時拋出的異常
    Anime: 動畫下載主類別，處理影片解析、下載、上傳等操作
"""
...
class Anime:
    ...
    Attributes:
        ...
        _settings: 配置設定
        _cookies: Cookie 資訊
        video_resolution: 影片解析度
        video_size: 影片檔案大小（MB）
        local_video_path: 本地影片路徑
    """

    def __init__(
        self, sn: int | str, debug_mode: bool = False, gost_port: int = 34173
    ) -> None:
        """初始化 Anime 物件。

        Args:
            sn: 影片序號（SN碼）
            debug_mode: 除錯模式，啟用時不會執行實際下載
            gost_port: Gost 代理伺服器埠號
        """
        self._cfg = config.get_config()
        self._settings = config.get_settings()  # 使用新的 Settings dataclass
    ...
    def __init_proxy(self):
        if self._settings.use_gost:
            ...
            self._proxies = {
                "https": "http://127.0.0.1:" + self._gost_port,
                "http": "http://127.0.0.1:" + self._gost_port,
            }
        else:
            # 无需 gost 的情况
            os.environ["HTTP_PROXY"] = self._cfg.proxy
    ...
    def get_filename(self):
        if self.video_resolution == 0:
            return self.__get_filename(self._cfg.download_resolution)
        else:
    ...
    def __get_src(self):
        if self._cfg.use_mobile_api:
            ...
            f = self.__request(req, no_cookies=True, use_httpx=True)
            self._src = BeautifulSoup(f.content, "html.parser")
    ...
    def __get_episode(self):  # 提取集数
        def get_ep():
            ...
            self._episode = re.findall(
                r"\[\d*\.?\d* *\.?[A-Z,a-z]*(?:電影)?\]", self._title
            )
            ...
            for _type in self._src["data"]["anime"]["episodes"]:
                for _sn in self._src["data"]["anime"]["episodes"][_type]:
                    if _type == "0":  # 本篇
                        ...
                        self._episode_list[f"特別篇{_sn['episode']}"] = int(
                            _sn["videoSn"]
                        )
                    elif _type == "3":  # 中文配音
                        self._episode_list[f"中文配音{_sn['episode']}"] = int(
                            _sn["videoSn"]
                        )
                    else:  # 中文電影
            ...
            try:
                ...
                for i in a:
                    ...
                    if ep in self._episode_list.keys():
                        ...
                        # 檢查索引是否在範圍內，避免 IndexError
                        if len(p) > 0 and index_counter[ep] < len(p):
                            ep = p[index_counter[ep]] + ep
                        else:
                            # 如果沒有對應的標籤或索引超出範圍，使用計數器標記
                            ep = f"#{index_counter[ep] + 1} " + ep
    ...
    def __init_header(self):
        ...
        origin = "https://" + host
        ua = self._cfg.ua  # cookie 自动刷新需要 UA 一致
    ...
    def __request(
        ...
        while True:
            try:
                if use_httpx:
                    ...
                    if self._proxies:
                        # httpx uses 'proxies' parameter (similar to requests)
                        proxy_url = self._proxies.get("http") or self._proxies.get(
                            "https"
                        )
                        f = self._httpx_client.get(
                            req,
                        ...
                        f = self._httpx_client.get(
                            req, headers=current_header, cookies=cookies
                        )
                else:
                    f = self._httpx_client.get(
                        req, headers=current_header, cookies=cookies
        ...
        if not self._cookies:
            ...
            "nologinuser" not in self._cookies.keys()
            and "BAHAID" not in self._cookies.keys()
        ):
            # 处理遊客cookie
            if "nologinuser" in self._httpx_client.cookies.keys():
                # self._cookies['nologinuser'] = self._httpx_client.cookies['nologinuser']
                self._cookies = dict(self._httpx_client.cookies)
            ...
            if "set-cookie" in f.headers.keys():  # 发现server响应了set-cookie
                if "deleted" in f.headers.get("set-cookie"):
                    ...
                    if (
                        ...
                        err_print(
                            self._sn, "嘗試切換回 Web Header 刷新 Cookie", display=False
                        )
                        self._req_header = self._web_header
                        self.__request(
                        ...
                        if not succeed_flag:
                            if self._cfg.disable_guest_mode:
                                ...
                                raise TryTooManyTimeError("Cookie 更新失敗，且已設定不使用遊客帳號")

                            # 使用遊客模式：清空 cookies
                            self._cookies = {}
                            err_print(
                                0,
                                "用戶cookie更新失敗! 使用遊客身份訪問",
                                status=1,
                                no_sn=True,
                            )
                        ...
                        if (
                            self._cfg.use_mobile_api
                            and "X-Bahamut-App-Android" not in self._req_header
                        ):
                    ...
                    err_print(
                        self._sn, f"用戶cookie刷新 {key_list_str} ", display=False
                    )
    ...
    def __request_json(
        ...
        response = self.__request(
            req, no_cookies, show_fail, max_retry, addition_header, use_httpx
        )
        # Both httpx and requests use .json() method
    ...
    def __get_m3u8_dict(self):
        ...
        def get_playlist():
            if self._cfg.use_mobile_api:
                ...
                req = (
                    "https://ani.gamer.com.tw/ajax/m3u8.php?sn="
                    + str(self._sn)
                    + "&device="
                    + self._device_id
                )
        ...
        def gain_access():
            if self._cfg.use_mobile_api:
                ...
                req = (
                    "https://ani.gamer.com.tw/ajax/token.php?adID=0&sn="
                    + str(self._sn)
                    + "&device="
                    + self._device_id
                    + "&hash="
        ...
        def unlock():
            req = (
                "https://ani.gamer.com.tw/ajax/unlock.php?sn="
                + str(self._sn)
                + "&ttl=0"
            )
        ...
        def check_lock():
            req = (
                "https://ani.gamer.com.tw/ajax/checklock.php?device="
                + self._device_id
                + "&sn="
                + str(self._sn)
            )
        ...
        def start_ad():
            if self._cfg.use_mobile_api:
                ...
                req = (
                    "https://ani.gamer.com.tw/ajax/videoCastcishu.php?sn="
                    + str(self._sn)
                    + "&s=194699"
                )
            f = self.__request(req)  # 无响应正文
        ...
        def check_no_ad(error_count=10):
            ...
            req = (
                "https://ani.gamer.com.tw/ajax/token.php?sn="
                + str(self._sn)
                + "&device="
                + self._device_id
                + "&hash="
            ...
            if "time" in resp.keys():
                if not resp["time"] == 1:
                    ...
                    if error_count != 10:
                        ...
                        config.save_config(self._cfg)  # 保存到配置文件
            else:
                err_print(
                    self._sn, "遭到動畫瘋地區限制, 你的IP可能不被動畫瘋認可!", status=1
                )
                sys.exit(1)
        ...
        def parse_playlist():
            ...
            f = self.__request(
                playlist_url,
                no_cookies=True,
                addition_header={"origin": "https://ani.gamer.com.tw"},
            )
        ...
        if not user_info["vip"]:
            ...
            if self._cfg.use_mobile_api:
                ad_time = self._cfg.mobile_ads_time  # APP解析廣告解析時間不同
            else:
                ad_time = self._cfg.ads_time
    ...
    def __get_filename(self, resolution, without_suffix=False):
        # 处理剧集名补零
        if (
            re.match(r"^[+-]?\d+(\.\d+){0,1}$", self._episode)
            and self._cfg.zerofill > 1
        ):
            # 正则考虑到了带小数点的剧集
            # 如果剧集名为数字, 且用户开启补零
            if re.match(r"^\d+\.\d+$", self._episode):
                ...
                episode = a.zfill(self._cfg.zerofill) + b
            else:
                # 如果是整数
                episode = self._episode.zfill(self._cfg.zerofill)
        else:
            episode = self._episode

        if self._cfg.plex_naming:
            # 適配 PLEX 命名規則
            ...
            if season:
                ...
                episode = (
                    "[S"
                    + str(season_num).zfill(self._cfg.zerofill)
                    + "E"
                    + episode
                    + "]"
                )
                ...
                episode = (
                    "[E" + episode + "]"
                )  # do not classify season if the bangumi type is "特別篇" or "中文配音"
            elif episode == "電影":
                episode = "[" + episode + "]"  # there is no episode num for "電影"
            else:
                episode = (
                    "[S01E" + episode + "]"
                )  # as season 1 if there is no matching above types
        else:
            episode = "[" + episode + "]"
        ...
        if self._cfg.add_bangumi_name_to_video_filename:
            ...
            # 如果用户不要将番剧名添加到文件名
            filename = self._cfg.customized_video_filename_prefix + episode
    ...
    def __segment_download_mode(self, resolution=""):
        ...
        temp_dir = os.path.join(
            self._temp_dir, str(self._sn) + "-downloading-by-aniGamerPlus"
        )  # 临时目录以 sn 命令
        ...
        m3u8_text = self.__request(
            self._m3u8_dict[resolution], no_cookies=True
        ).text  # 请求 m3u8 文件
        with open(m3u8_path, "w", encoding="utf-8") as f:  # 保存 m3u8 文件在本地
            f.write(m3u8_text)
            pass
        key_match = re.search(r'(?<=AES-128,URI=")(.*)(?=")', m3u8_text)
        if not key_match:
            raise ValueError(f"無法從 m3u8 文件中提取 key URI: {m3u8_text[:200]}")
        key_uri = key_match.group()  # 把 key 的链接提取出来
        ...
        limiter = threading.Semaphore(
            self._cfg.multi_downloading_segment
        )  # chunk 并发下载限制器
        ...
        m3u8_text_local_version = m3u8_text.replace(
            original_key_uri, os.path.join(temp_dir, "key.m3u8key")
        ).replace("\\", "\\\\")
        for chunk in chunk_list:
    ...
    def __ffmpeg_download_mode(self, resolution=""):
        ...
        if os.path.exists(downloading_file):
            os.remove(downloading_file)  # 清理任務失败的尸体
        ...
        if run_ffmpeg.returncode == 0 and (
            ...
            err_msg_detail = (
                filename
                + " ffmpeg_return_code="
                + str(run_ffmpeg.returncode)
                + " Bad segment="
                + str(return_str.find("Failed to open segment"))
            )
            err_print(self._sn, "下載失败", err_msg_detail, status=1)
    ...
    def download(
        ...
        if not resolution:
            resolution = self._cfg.download_resolution

        if save_dir:
            self._bangumi_dir = save_dir  # 用于 cui 用户指定下载在当前目录

        # 預先保留原始標題
        self._bangumi_name_orig = self._title.replace(
            "[" + self.get_episode() + "]", ""
        ).strip()  # 提取番剧名（去掉集数后缀）
        self._bangumi_name_orig = re.sub(
            r"\s+", " ", self._bangumi_name_orig
        )  # 去除重复空格
        ...
        # 下载任務开始
        config.tasks_progress_rate[int(self._sn)] = {
            "rate": 0,
        ...
        if (
            check_ffmpeg.stdout and check_ffmpeg.stdout.readlines()
        ):  # 查找 ffmpeg 是否已放入系统 path
        ...
        if classify:  # 控制是否建立番剧文件夹
            if self._cfg.classify_season:  # 控制是否建立番剧季度子文件夹
                season = re.findall(self.season_title_filter, self._bangumi_name_orig)
                extra = re.findall(self.extra_title_filter, self._bangumi_name_orig)
                if season:
                    season_num_string = (
                        "".join(season).replace("第", "").replace("季", "")
                    )
                    season_num = self.get_season_num(season_num_string)
                    root_bangumi_dir = self._bangumi_name_orig.replace(
                        str(season[0]), ""
                    )  # remove season name for bangumi root dir
                    root_bangumi_dir = "".join(
                        root_bangumi_dir.rstrip()
                    )  # remove tail space if exists
                    sub_dir = "Season " + str(season_num)  # add season sub folder
                elif extra:
                    root_bangumi_dir = self._bangumi_name_orig.replace(
                        "[" + str(extra[0]) + "]", ""
                    )  # remove extra name for bangumi root dir
                    root_bangumi_dir = "".join(
                        root_bangumi_dir.rstrip()
                    )  # remove tail space if exists
                    ...
                    root_bangumi_dir = self._bangumi_name_orig.replace(
                        "[電影]", ""
                    )  # remove 電影 for bangumi root dir
                    sub_dir = (
                        "Movie"  # add movie sub folder if the bangumi type is "電影"
                    )
                else:
                ...
                self._bangumi_dir = os.path.join(
                    self._bangumi_dir,
                    config.legalize_filename(root_bangumi_dir),
                    sub_dir,
                )
            else:
                self._bangumi_dir = os.path.join(
        ...
        if resolution not in self._m3u8_dict.keys():
            if self._cfg.lock_resolution:
                # 如果用户设定锁定清晰度, 則下載取消
                err_msg_detail = (
                    "指定清晰度不存在, 因當前鎖定了清晰度, 下載取消. 可用的清晰度: "
                    + "P ".join(self._m3u8_dict.keys())
                    + "P"
                )
        ...
        if self._cfg.segment_download_mode:
            self.__segment_download_mode(resolution)
        else:
            self.__ffmpeg_download_mode(resolution)

        # 任務完成, 记录到已完成列表并从进度表中删除
        config.record_completed_task(int(self._sn), self.get_filename(), "success")
        ...
        if self._danmu:
            try:
                full_filename = os.path.join(
                    self._bangumi_dir, self.__get_filename(resolution)
                ).replace("." + self._cfg.video_filename_extension, ".ass")
                d = Danmu(self._sn, full_filename, config.read_cookie())
                d.download(self._cfg.danmu_ban_words)
            except BaseException as e:
                err_print(
                    self._sn, "彈幕異常", "下載彈幕時發生未知錯誤: " + str(e), status=1
                )
                err_print(
                    self._sn,
                    "彈幕異常",
        ...
        if self._cfg.coolq_notify:
            try:
                ...
                if self._cfg.coolq_settings.message_suffix:
                    # 追加用户信息
                    msg = msg + "\n\n" + self._cfg.coolq_settings.message_suffix

                for query in self._cfg.coolq_settings.query:
                    if "?" not in query:
                        query = query + "?"
                    else:
                        query = query + "&"
                    req = (
                        query
                        + self._cfg.coolq_settings.msg_argument_name
                        + "="
                        + quote(msg)
                    )
        ...
        if self._cfg.telebot_notify:
            try:
                ...
                vApiTokenTelegram = self._cfg.telebot_token
                try:
                    if (
                        self._cfg.telebot_use_chat_id and self._cfg.telebot_chat_id
                    ):  # 手动指定发送目标
                        chat_id = self._cfg.telebot_chat_id
                    else:
                        ...
                        chat_id = response["result"][0]["message"]["chat"][
                            "id"
                        ]  # Get chat id
                    try:
        ...
        if self._cfg.discord_notify:
            try:
                ...
                err_print(
                    self._sn,
                    "Discord NOTIFY UNKNOWN ERROR",
                    "Exception: " + str(e),
                    status=1,
                )

        # plex 自動更新媒體庫
        if self._cfg.plex_refresh:
            try:
                url = "https://{plex_url}/library/sections/{plex_section}/refresh?X-Plex-Token={plex_token}".format(
                    plex_url=self._cfg.plex_url,
                    plex_section=self._cfg.plex_section,
                    plex_token=self._cfg.plex_token,
                )
                r = httpx.get(url)
                ...
                err_print(
                    self._sn,
                    "Plex auto Refresh UNKNOWN ERROR",
                    "Exception: " + str(e),
                    status=1,
                )
    ...
    def upload(self, bangumi_tag="", debug_file=""):
        ...
        def connect_ftp(show_err=True):
            ...
            while err_counter <= 3:
                try:
                    ftp.connect(self._cfg.ftp.server, self._cfg.ftp.port)  # 连接 FTP
                    ftp.login(self._cfg.ftp.user, self._cfg.ftp.pwd)  # 登陆
                    connect_flag = True
                    break
            ...
            if self._cfg.ftp.cwd:
                try:
                    ftp.cwd(self._cfg.ftp.cwd)  # 进入用户指定目录
            ...
            try:
                ...
                try:
                    ...
                    if show_err:
                        detail = (
                            "你可能沒有權限創建目錄(用於分類番劇), 影片文件將會直接上傳, 收到異常: "
                            + str(e)
                        )
                        err_print(self._sn, "FTP狀態", detail, status=1)
        ...
        def exit_ftp(show_err=True):
            try:
                ...
                if show_err and self._cfg.ftp.show_error_detail:
                    err_print(
                        self._sn,
                        "FTP狀態",
        ...
        err_print(
            self._sn, "正在上傳", self._video_filename + " title=" + self._title + "……"
        )
        try_counter = 0
        video_filename = (
            self._video_filename
        )  # video_filename 将可能会储存 pure-ftpd 缓存文件名
        max_try_num = self._cfg.ftp.max_retry_num
            ...
            try:
                if try_counter > 0:
                    ...
                    try:
                        ...
                        if (
                            not str(resp) == "550 No files found"
                        ):  # 非文件不存在错误, 抛出异常
                            raise
                # 断点续传
                try:
                ...
                if remote_size != local_size:
                    ...
                    detail = (
                        self._video_filename
                        + " 在遠端為"
                        + str(round(remote_size / float(1024 * 1024), 2))
                        + "MB"
                        + " 與本地"
                        + str(round(local_size / float(1024 * 1024), 2))
                        + "MB 不一致! 將重試最多"
                        + str(max_try_num)
                        + "次"
                    )
                ...
                try:
                    ...
                    pass
                ftp.rename(
                    tmp_dir + "/" + video_filename, self._video_filename
                )  # 将视频从临时文件移出, 顺便重命名
        ...
        if not self.upload_succeed_flag:
            err_print(
                self._sn, "上傳失敗", self._video_filename + " 放棄上傳!", status=1
            )
            exit_ftp()
            return self.upload_succeed_flag
